...
- **2026-09-25: Golden wire-format files (R6), CODEOWNERS + real semantic PR-title action,
  ...
  generated by the same tests (`UPDATE_GOLDEN=1`). Volatile fields (`session_id`,
  `duration_ms`) are normalized before comparison so the goldens pin the *shape*, not a run.
  This catches accidental contract changes (renamed JSON tags, dropped `omitempty`) that
  field-level assertions miss. The `pr-title` job now uses the real
  `amannn/action-semantic-pull-request` action rather than a shell regex, scoped with
  `if: github.event_name == 'pull_request'`; a skipped job still reports a success-equivalent
  status, so it stays safe to require. The nightly workflow is deliberately **not** required
  CI: `scripts/container_e2e.sh` builds and drives the shipped image (this is the "does the
  artifact work" check, not "does the source compile"), and `scripts/eval_live.sh` replays
  eval cases against a real model. The container E2E runs the LLM stub as a **container on a
  shared Docker network**, not as a host process: a host firewall (UFW/nftables `INPUT DROP`)
  drops container-to-host traffic, so `host.docker.internal`/`--add-host=host-gateway` is
  unreliable on a locked-down host. Container-to-container by service name has no such
  dependency and works anywhere Docker does. `eval_live` skips (does not fail) without a key,
  so a fork or secretless repo stays green.
