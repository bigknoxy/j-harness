...
// addr returns the Redis address for tests, honoring HARNESS_REDIS_ADDR.
func addr() string {
	if v := os.Getenv("HARNESS_REDIS_ADDR"); v != "" {
		return v
	}
	return "127.0.0.1:6379"
}

// newTestStore connects to Redis or skips the test when it is unreachable. Each
// call uses a unique key prefix so tests never collide.
func newTestStore(t *testing.T) *RedisStore {
	t.Helper()
	probe, err := Open(Config{Addr: addr()})
	if err != nil {
	...
	s, err := Open(Config{
		Addr:      addr(),
		KeyPrefix: fmt.Sprintf("jhtest:%d:%s:", time.Now().UnixNano(), t.Name()),
	})
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
...
func TestCreateAndGetJob(t *testing.T) {
	...
	job := model.Job{SessionID: "s1", Kind: model.KindAgent, TargetID: "triage", Input: "hello"}
...
func TestGetJobNotFound(t *testing.T) {
	s := newTestStore(t)
	if _, err := s.GetJob(context.Background(), "nope"); err != store.ErrNotFound {
...
func TestSetJobStatus(t *testing.T) {
	...
	if err := s.SetJobStatus(ctx, "missing", model.StatusFailed, "", "x"); err != store.ErrNotFound {
...
func TestOpenBadAddress(t *testing.T) {
	// A closed port should fail fast rather than hang.
	if _, err := Open(Config{Addr: "127.0.0.1:1"}); err == nil {
		t.Fatal("expected Open to fail for an unreachable address")
	}
}

// TestStoreInterface guards that both adapters satisfy the same contract.
func TestStoreInterface(t *testing.T) {
	if _, err := Open(Config{Addr: addr()}); err != nil {
